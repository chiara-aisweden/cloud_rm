...
class QuantileNetworkMM(nn.Module):
    ...
    def predict(self,x):
        ...
        if self.data_norm:
            ...
            means=torch.tensor(self.X_means,dtype=torch.float,device=self.device)
            stds=torch.tensor(self.X_stds,dtype=torch.float,device=self.device)

            tX=torch.tensor(x,dtype=torch.float,device=self.device)
            tX=torch.tensor((tX - means) / stds,dtype=torch.float,device=self.device)
